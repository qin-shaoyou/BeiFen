...
public abstract class WorkTask<Params, Progress, Result> {
	...
	private Result doInBackground(Params... params) {
		// Log.d(TAG, String.format("%s --->doInBackground()",
		// TextUtils.isEmpty(taskId) ? "run " : (taskId + " run ")));
	...
	final protected void onPreExecute() {
		// Log.d(TAG, String.format("%s --->onTaskStarted()",
		// TextUtils.isEmpty(taskId) ? "run " : (taskId + " run ")));
		onPrepare();
	}
...
//				resultIsNull();
//			} else {
			// Log.d(TAG, String.format("%s --->onTaskSuccess()",
			// TextUtils.isEmpty(taskId) ? "run " : (taskId + " run ")));
				onSuccess(result);
//			}
		}
		...
		// Log.d(TAG, String.format("%s --->onTaskComplete()",
		// TextUtils.isEmpty(taskId) ? "run " : (taskId + " run ")));
		onFinished();
	}
