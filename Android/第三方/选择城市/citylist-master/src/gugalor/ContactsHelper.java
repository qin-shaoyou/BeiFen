...
public class ContactsHelper {
	...
	public void parseT9InputSearchContacts(String search) {
		...
		if (null == search) {// add all base data to search
			...
			for (int i = 0; i < mBaseContacts.size(); i++) {
				...
				for (currentContacts = mBaseContacts.get(i); null != currentContacts; currentContacts = currentContacts
						.getNextContacts()) {
					currentContacts
							.setSearchByType(Contacts.SearchByType.SearchByNull);
		...
		for (int i = 0; i < contactsCount; i++) {
			...
			if (true == T9MatchPinyinUnits.matchPinyinUnits(pinyinUnits, name,
				...
				for (currentContacts = mBaseContacts.get(i), firstContacts = currentContacts; null != currentContacts; currentContacts = currentContacts
						.getNextContacts()) {
					currentContacts
							.setSearchByType(Contacts.SearchByType.SearchByName);
					currentContacts.setMatchKeywords(chineseKeyWord.toString());
					currentContacts
							.setMatchStartIndex(firstContacts.getName()
									.indexOf(
											firstContacts.getMatchKeywords()
													.toString()));
					currentContacts.setMatchLength(firstContacts
							.getMatchKeywords().length());
	...
	public void showContactsInfo() {
		...
		for (int i = 0; i < contactsCount; i++) {
			...
			for (currentCoutacts = ContactsHelper.getInstance()
					.getBaseContacts().get(i); null != currentCoutacts; currentCoutacts = currentCoutacts
					.getNextContacts()) {
				Log.i(TAG,
						"======================================================================");
				String name = currentCoutacts.getName();
				List<PinyinUnit> pinyinUnit = currentCoutacts
						.getNamePinyinUnits();
	...
	private List<Contacts> loadContacts(Context context) {
		...
		try {
			...
			int areaCode_index = cursor.getColumnIndex("phoneCode");
			...
			while (cursor.moveToNext()) {
				...
				String displayName = cursor.getString(nameindex) + "-"
						...
						+ cursor.getString(province_index) + "--区号:"
						+ cursor.getString(areaCode_index);

				boolean kanjiStartContactsExist = kanjiStartContactsHashMap
						.containsKey(id);
				boolean nonKanjiStartContactsExist = nonKanjiStartContactsHashMap
						.containsKey(id);
				...
				if (true == kanjiStartContactsExist) {
					...
					boolean isKanji = PinyinUtil
							.isKanji(cs.getName().charAt(0));
		...
		for (int i = 0; i < nonKanjiStartContacts.size(); i++) {
			String nonKanfirstLetter = PinyinUtil
					.getFirstLetter(nonKanjiStartContacts.get(i)
							.getNamePinyinUnits());
			// Log.i(TAG, "nonKanfirstLetter=["+nonKanfirstLetter+"]");
			int j = 0;
			for (j = 0 + lastIndex; j < contacts.size(); j++) {
				String firstLetter = PinyinUtil.getFirstLetter(contacts.get(j)
						.getNamePinyinUnits());
				lastIndex++;
		...
		Log.i(TAG, "endLoadTime-startLoadTime=["
				+ (endLoadTime - startLoadTime) + "] contacts.size()="
				+ contacts.size());
